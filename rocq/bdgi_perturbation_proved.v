...
(* ================================================================== *)
(*  Section 9: Composition Interface and Proved Rupture Decomposition  *)
(*                                                                      *)
(*  Architecture: neutral in interface, concrete in witness.           *)
(*  The abstract module type states what any composition operator      *)
(*  must satisfy. The concrete module provides one synergy-aware       *)
(*  instance and carries the proof. rupture_decomposition is proved    *)
(*  for that instance; the abstract parameter in Section 5 documents   *)
(*  the general principle over arbitrary operators.                    *)
(* ================================================================== *)

(* ------------------------------------------------------------------ *)
(*  9.1  Abstract composition interface                                 *)
(* ------------------------------------------------------------------ *)

Module Type COMPOSITION_SIG.

  Parameter par_compose : Perturbation -> Perturbation -> Perturbation.
  Parameter synergy_debt : Perturbation -> Perturbation -> Prop.

  (*  EMG propagation: if either component is emergent, so is the     *)
  (*  composition. The independent channel cannot be stripped by       *)
  (*  composition.                                                     *)
  ...
  (*  Magnitude monotonicity: the composite upper bound is at least   *)
  (*  as large as either component's upper bound.                     *)
  ...
  (*  Rupture decomposition principle: rupture of a composite implies  *)
  (*  rupture of a component, or synergy debt.                         *)
...
(* ------------------------------------------------------------------ *)
(*  9.2  Concrete instance: SimpleSynergyCompose                        *)
(*                                                                      *)
(*  One synergy-aware operator that satisfies COMPOSITION_SIG.         *)
(*  Synergy debt is the condition that both components are individually *)
(*  tolerated but their composition is not. All three axioms are        *)
(*  proved here without Admitted.                                       *)
(* ------------------------------------------------------------------ *)

Module SimpleSynergyCompose <: COMPOSITION_SIG.

  (*  BType dominance: EMG > REL > INV.                                *)
  Definition btype_join (b1 b2 : BType) : BType :=
    match b1, b2 with
    | EMG, _ => EMG
    | _, EMG => EMG
    | REL, _ => REL
  ...
  (*  Magnitude envelope: lower bound is min, upper bound is max.      *)
  Definition iv_envelope (a b : Interval) : Interval.
  Proof.
    refine (mkInterval (Rmin (iv_lo a) (iv_lo b))
                       (Rmax (iv_hi a) (iv_hi b)) _).
    apply Rle_trans with (r2 := iv_lo a).
    - apply Rmin_l.
    - apply Rle_trans with (r2 := iv_hi a).
      + exact (iv_wf a).
      + apply Rmax_l.
  Defined.

  Definition par_compose (p q : Perturbation) : Perturbation :=
    mkPerturbation
      (btype_join (p_btype p) (p_btype q))
      (p_carrier p)
      (iv_envelope (p_magnitude p) (p_magnitude q))
      (p_mode p).

  (*  Synergy debt: both tolerated, composite ruptured.               *)
  Definition synergy_debt (p q : Perturbation) : Prop :=
    exists tol : TolPredicate,
      tol p /\ tol q /\ Rupture tol (par_compose p q).

  Lemma compose_btype_emg :
    forall p q,
      p_btype p = EMG \/ p_btype q = EMG ->
      p_btype (par_compose p q) = EMG.
  Proof.
    intros p q [Hp | Hq]; simpl; unfold btype_join.
    - rewrite Hp. reflexivity.
    - rewrite Hq. destruct (p_btype p); reflexivity.
  Qed.

  Lemma compose_magnitude_ub :
    forall p q,
      (iv_hi (p_magnitude p) <= iv_hi (p_magnitude (par_compose p q)))%R /\
      (iv_hi (p_magnitude q) <= iv_hi (p_magnitude (par_compose p q)))%R.
  Proof.
    intros p q. simpl. unfold iv_envelope. simpl. split.
    - apply Rmax_l.
    - apply Rmax_r.
  Qed.

  (*  The core proof. Classical reasoning over tolerance decisions:    *)
  (*  if both components are tolerated, the composite's rupture must   *)
  (*  arise from their interaction — that is what synergy_debt names.  *)
  ...
  Proof.
    ...
    - destruct (classic (tol q)) as [Hq | Hnq].
      + (*  Both individually tolerated; composite ruptured.           *)
        (*  This is exactly synergy debt: the interaction is the cause.*)
        right. right.
        unfold synergy_debt. exists tol.
        exact (conj Hp (conj Hq Hrup)).
      + right. left. exact Hnq.
    - left. exact Hnp.
  Qed.

End SimpleSynergyCompose.

(* ------------------------------------------------------------------ *)
(*  9.3  Proved rupture decomposition                                   *)
(*                                                                      *)
(*  This is the concrete witness satisfying the abstract principle     *)
(*  stated in Section 5. The third disjunct names the interaction      *)
(*  term explicitly: both components tolerated, composite ruptured.   *)
(* ------------------------------------------------------------------ *)
