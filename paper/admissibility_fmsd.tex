...
admissibility. A predicate is admissible exactly when it factors through the
observational quotient.
Equivalently, the quotient must preserve every distinction required by
the predicate. For perturbation systems, we introduce warrant debt as an
...
empirical tolerance predicate is not admissible. The mechanisation treats inadmissibility as a proof obligation prior to
proof search. The Rocq development
...
Theorem. The abstract composition theorem is stated as an interface condition.
The concrete instance satisfying that interface is proved. A higher-order dependency extension is presented as a mathematical
...
safety predicate is well formed relative to an observation interface.
It gives three equivalent characterisations: factorisation through the
observation map, constancy on equivalence classes, and partition
refinement.
This theorem is the criterion used by the rest of the paper.
...
The mechanised development then separates two further issues.
Section~\ref{sec:composition} proves a concrete binary rupture
decomposition theorem for \texttt{SimpleSynergyCompose}.
Section~\ref{sec:boundary} proves the Non-Factorisation Theorem, which
rules out total algebraic projections that preserve rupture behaviour
in general. A higher-order extension to finite dependency
...
Theorem~\ref{thm:factorisation} is the central criterion used throughout the paper.
...
This section introduces the formal objects required for the Warrant
Debt Theorem of Section~\ref{sec:warrant}.
...
The distinction is representational.
\texttt{EMG} marks the case in which algebraic observation is
...
relation.
Emergent perturbations lie outside its domain: since
\texttt{btype\_algebraic(EMG) = false}, no emergent perturbation is
related even to itself.
...
observation map.
The correct response is refinement of the observation map, not renewed
proof search inside the same quotient.
\end{example}
...
Accordingly, global rupture can be analysed through the disjunctive
family of support witnesses
\[
\mathrm{Rup}(\Phi_{\mathrm{global}})
\;\Rightarrow\;
\bigvee_{\emptyset \neq S \subseteq C}
\Sigma_S,
\]
where $\Sigma_S$ records that $S$ supports a witness of inadmissibility.
The question is not whether some $\Sigma_S$ appears.
The question is which $S$ is minimal.
...
This definition is schematic: a full formalisation requires explicit
projection maps $M_S$ and projected predicates $\Phi|_S$ for each
subsystem $S \subseteq C$.

\end{definition}

A predicate may be admissible on every proper projection of size less
than $k$, yet fail to be admissible on the full system.
This is $k$-th order synergy.

\begin{conjecture}[Higher-Order Warrant Debt]
\label{thm:higher-order}

Let $\Phi$ be a global predicate on a finite component family
$C = \{C_1,\ldots,C_n\}$, with admissibility-induced dependency family
$\mathcal{D}(\Phi)$.
Suppose that $\Phi$ is inadmissible relative to the observation map on
$C$.
If, for some $k \leq n$, every projected predicate $\Phi|_T$ is
admissible for every subsystem $T \subseteq C$ with $|T| < k$, then
there exists a minimal support set $S^* \in \mathcal{D}(\Phi)$ with
$|S^*| \geq k$.
...
The claim is not that the present paper models those domains in detail,
but that the admissibility obligation identifies a design condition any
such monitor must satisfy.

The Rocq-certified warrant-debt witness makes this failure auditable.
...
In an industrial safety lifecycle, the witness has three uses.
Before deployment, it can reject an insufficient monitor design.
During operation, it can trigger structural review rather than ordinary
anomaly triage.
After failure, it can identify which observational boundary must be
refined.
...
Only then can verification assign warrant debt: the failed
observational boundary, the required refinement, and the subsystem,
interface, or design assumption to be reviewed.

\begin{remark}[Strengthened Rupture-to-Synergy Bridge]
\label{cor:bridge}

Assuming the higher-order dependency-family structure of
Conjecture~\ref{thm:higher-order}, a global rupture whose lower-order
projections do not suffice determines a minimal support set
$S^* \in \mathcal{D}(\Phi)$ of order at least $k$ that witnesses
higher-order warrant debt.
...
The strengthened formulation replaces an unclassified residual with a
formal object: the minimal witness of inadmissibility, located at a
definite level of the dependency hierarchy.

The two mechanised results, Theorem~\ref{thm:wdt} and
Theorem~\ref{thm:rupture}, separate representational failure from
binary compositional failure.
Conjecture~\ref{thm:higher-order} indicates how this separation should
extend to higher-order dependency families, but that extension is not
claimed as a mechanised result of the present paper.
...
development.
In particular, the theorem \texttt{no\_alg\_factorisation} is
established in the canonical development file
\texttt{bdgi\_perturbation\_proved.v}.
It proves, by explicit witness construction, that rupture cannot be
uniformly preserved by projection into the algebraic fragment.
This removes the earlier conjectural status of non-factorisation.
It also makes the limitation structural, not epistemic.
...
is not valid, and the corresponding lemma has been removed from
...
depends, without the invalid universal quantification over admissibility
predicates present in the earlier formulation.
\end{remark}
...
For verification systems mediated by an observation interface,
admissibility is prior to proof.
Only after the target predicate is shown to respect the observational
quotient does proof search address the predicate itself.
...
The higher-order extension is not mechanised here.
It indicates how the binary result should generalise to dependency
families.
In that setting, a global failure is classified by a minimal support
set.
That set identifies the smallest subsystem on which the global
predicate depends, and hence the smallest subsystem, interface, or
design assumption requiring review.
