...
\title{Associator Fields and Local-to-Global Failure\\ in Finite Regional Cohomology}
...
\begin{abstract}
Local compatibility does not guarantee global gluing. In particular, pairwise-compatible boundary corrections do not, in general, guarantee coherent ternary gluing: an algebraic operation assembled from regional data may be associative on every region and explicitly corrected on every pairwise overlap while threefold composition still depends on parenthesisation. This paper introduces the associator field, an assignment of a boundary-supported defect to each non-empty triple overlap of a finite regional cover, and proves that under support-refining boundary corrections the defect of a corrected triple product is supported on the triple overlap. The field is treated as the primitive datum, and its classification is separated into three distinct questions: detection asks where the field is non-zero, repair asks whether it can be removed by admissible changes of pairwise boundary bookkeeping, and obstruction asks whether a persistent component remains after all such repairs. A non-zero defect is therefore not automatically an obstruction. The paper then gives a computable finite certificate of genuine obstruction: a four-region seam residue \(r=(1,1,1,-2)\) over \(\Q\) is shown to be a cocycle but not a coboundary, hence to define a non-zero class in \(H^1\), with the proof given both by an explicit linear contradiction and by a non-zero cycle pairing. This paper establishes persistence of obstructions under any single admissible refinement. A stronger presentation-invariance theory would compare two different presentations through a common admissible refinement. That problem is not addressed here but is the subject of future work.
\end{abstract}

\section{Introduction}

Pairwise-compatible boundary corrections do not, in general, guarantee coherent ternary gluing. That sentence is the governing claim of this paper, and everything below either makes it precise, proves it in a controlled finite setting, or certifies a case in which the resulting failure cannot be repaired.

Associativity is usually presented as an internal algebraic law: an algebra is associative if \((ab)c=a(bc)\) for all elements. That formulation hides a boundary problem. If an algebraic operation is assembled from local data on overlapping regions, then there are two separate questions. First, does multiplication associate inside each region? Second, does multiplication continue to associate after local products have been glued across seams? The second question is not a restatement of the first. A gluing system may have associative algebras on every local region and explicit correction terms on every pairwise overlap, and threefold composition may nevertheless depend on parenthesisation. Schematically, the defect has the form
\[
...
The paper studies that residue directly. For a finite cover \(\mathcal U=(U_i)_{i\in I}\), the primitive datum is the associator field
\[
(i,j,k) \longmapsto \alpha_{ijk},
\]
where \(\alpha_{ijk}\) is the associator defect associated with the triple \((U_i,U_j,U_k)\). Under the support hypotheses stated below, this defect is supported on the triple overlap \(U_i\wedge U_j\wedge U_k\). The failure is therefore localised, but it is not merely local: it is produced by comparing two histories of composition across a larger gluing pattern. Gluing thus creates a diagnostic level between pairwise compatibility and global coherence. A system can pass all unary checks, since each \(A(U_i)\) is internally associative. It can pass all binary checks, since each pairwise overlap \(U_i\wedge U_j\) carries an explicit boundary correction. It can still fail a ternary check, since the two ways of composing across \(U_i,U_j,U_k\) need not agree.

The finite example driving the paper is a four-region seam cycle with residue
\[
r=(1,1,1,-2)\in\Q^4 .
\]
The residue is closed but not exact. It is therefore not merely a detected defect; it is not removable by the declared local bookkeeping, and it represents a non-zero class in \(H^1\). The paper's abstract machinery is designed to explain why such a witness is the correct finite object to examine: the associator field detects ternary failure on triple seams, admissible pairwise repair is formulated as a constrained linear problem, and the residue left by the repair problem is classified by exact rational cohomology. Section~\ref{sec:actual-witness} proves the witness twice, by an explicit linear contradiction and by a non-zero cycle pairing, and Section~\ref{sec:invariance-refinement} shows that the verdict is stable under declared presentation changes and four declared refinement witnesses.

\subsection{Contributions}
\label{sec:contributions}

The paper makes three contributions, in increasing order of concreteness.

\begin{enumerate}[label=(\arabic*)]
\item \textbf{Associator-field localisation.} In a supported regional algebra with support-refining boundary corrections, the associator defect of a corrected triple product is supported on the triple overlap (Theorem~\ref{thm:triple-localisation}). For a finite cover and a family of local inputs, the localised defects assemble into a finite field indexed by the non-empty triple overlaps of the nerve.

\item \textbf{Repair before obstruction.} Detection, repair, and obstruction are distinct questions and are kept separate throughout. A non-zero associator field is not yet an obstruction: it may be removable by admissible changes of pairwise boundary bookkeeping. It becomes obstruction-like only when its class in the repair quotient is non-zero, that is, only after admissible pairwise repairs fail. The repair calculus is grounded in square-zero regional extensions (Theorem~\ref{thm:square-zero-regime}), in which the linearity of the repair operator is a consequence of the multiplication rule rather than an assumption. Cohomology enters as one possible classification of the field under additional coefficient, alternation, and coherence hypotheses, not as a structure the field carries automatically.

\item \textbf{Finite \(H^1\) certificate.} The four-region seam residue \(r=(1,1,1,-2)\) over \(\Q\) is a cocycle but not a coboundary, so it represents a non-zero \(H^1\) class (Proposition~\ref{prop:nonremovable}). The verdict carries two independent certificates, an inconsistent linear system and a non-zero cycle pairing, is invariant under five declared presentation changes, and persists under four declared refinement witnesses. These witnesses are then subsumed by a general admissible-refinement persistence theorem (Section~\ref{sec:admissible-refinement}).
\end{enumerate}

The order matters. The associator field comes first. Cohomological classification enters only after the field has been constructed and its support behaviour understood. The finite obstruction witness then shows how one such classification can be made exact and reproducible.

\subsection{Scope of the paper}

This paper does not give a general theory of descent, does not classify all gluing failures, does not prove presentation invariance via common refinement, and does not claim that every associator field is cohomological. It isolates a finite support-refined setting in which associator defects can be localised, repaired, and, in a declared finite case, certified as non-removable. The paper proves a universal admissible-refinement persistence theorem for refinements satisfying four declared algebraic conditions, and shows that the four refinement witnesses of the original work are instances of this theorem. The executable classifier accompanying the paper is exactly checked and hand-proved on the displayed witness; it has not been verified in a proof assistant, and Section~\ref{sec:scope-future} records that step as future work.

\subsection{Running example and reader's guide}

A single informal example recurs before being computed in Section~\ref{sec:first-order}. Suppose several nodes maintain interval views of a shared state. Their overlaps record shared interfaces, and pairwise boundary corrections record reconciliation costs at those interfaces. The central question is not whether each node can update its own data associatively, nor only whether every pair of nodes can reconcile. The question is whether the threefold history of reconciliation is independent of parenthesisation. This running example should not be confused with the general definition. It is an interpretive aid, not a premise of the theory. The formal theory works with finite distributive lattices, supported algebras, and bilinear boundary corrections; the interval model merely shows why the definitions have the shape they do.

Section~\ref{sec:related} positions the construction against coherence theory, descent, sheaf and cosheaf methods, and computational cohomology. Sections~\ref{sec:regions} and~\ref{sec:corrections} build the finite setting and prove the localisation theorem. Section~\ref{sec:dro} separates detection, repair, and obstruction, and Section~\ref{sec:first-order} gives the linearised repair calculus and grounds it in square-zero regional extensions, so that the linearity of repair is proved rather than assumed. Section~\ref{sec:certificates} supplies the bridge from triple-seam fields to seam-cycle residues and states the finite classifier. Sections~\ref{sec:actual-witness} and~\ref{sec:invariance-refinement} prove the witness and its declared stability. Section~\ref{sec:admissible-refinement} proves the witness-specific and universal admissible-refinement persistence theorems that subsume the four declared refinement witnesses. Section~\ref{sec:repro} makes the computational artefact reproducible, and Section~\ref{sec:scope-future} states the scope of the results and the open problems. Appendix~\ref{app:spectral} records a spectral diagnostic layer that is independent of the main line of argument.

\section{Related work}
\label{sec:related}

The construction of this paper sits between four literatures. Each supplies part of the vocabulary, and each differs from the present setting in a specific way that is worth stating precisely, because the paper's contribution is the intermediate level the four literatures leave open.
...
The contribution of this paper is not a new proof of descent, nor a replacement for \v{C}ech cohomology. It is a finite algebraic mechanism for producing, localising, and testing associator defects generated by boundary-corrected gluing. The paper's main novelty is the separation between the associator field as a primitive defect datum and its later classification by repair quotients or cohomology, together with a fully explicit finite witness in which that classification terminates in a certified non-removable obstruction.
...
\end{verbatim}
The input file declares the vertices, oriented edges, and residue vector of Figure~\ref{fig:fourcycle}. The emitted certificate is a JSON record with fields for the complex dimensions, the cocycle check \(\delta^1 r=0\), the solver status of \(\delta^0 b=r\), the cycle witness \(z\), and the rational pairing \(\langle z,r\rangle\); the certificates for the four refinement witnesses of Section~\ref{sec:invariance-refinement} are stored alongside the example inputs and record the pairings \(-7/2\), \(-4\), \(-5/4\), and \(-5\) of the table.

The division of labour between proof and computation is as follows. The obstruction verdict for the displayed residue is proved by hand twice, by the linear contradiction and by the cycle pairing of Section~\ref{sec:actual-witness}; the presentation-invariance and declared-refinement propositions are likewise proved in the text. The code is exactly checked over \(\Q\) with no floating point, and it is regression tested rather than formally verified: a property-based test runs \(1000\) random rational residues on the four-cycle and checks the classifier against the cycle-pairing criterion, under which \(\langle z,r\rangle=0\) exactly when \(r\) is removable and \(\langle z,r\rangle\neq 0\) exactly when \(r\) represents a nontrivial \(H^1\) obstruction on this complex. The current repository state records \(1000\) passed cases and zero failed cases. The computational artefact is therefore not a substitute for the proof. The proof of the actual residue is the explicit contradiction and the cycle pairing; the code provides reproducibility, regression protection, and a way of generating further finite witnesses.
...
The exact rational classifier, witness files, refinement certificates, and regression tests are available at \texttt{[https://github.com/dhwcmoore/associator-fields]}. The manuscript's finite obstruction witness can be reproduced by the command displayed above. No empirical data are required.
The exact rational classifier, witness files, refinement certificates, and regression tests are available at the repository accompanying this paper. The repository URL will be supplied in the final non-anonymised version. The manuscript's finite obstruction witness can be reproduced by the command displayed above. No empirical data are required.
...
The construction shows that pairwise interface data need not determine coherent ternary gluing. A finite regional system may be locally associative on every region and explicitly corrected on every pairwise seam while still producing a parenthesisation residue on a triple seam, and the associator field records that residue before it is forced into any particular classification scheme.

The paper has proved the localisation theorem: under support-refining boundary corrections, the associator defect of a corrected triple product is supported on the common triple overlap. It has separated detection, repair, and obstruction, with cohomology entering as a structured classification of the field rather than as an assumption built into it. And it has given an exact finite obstruction witness: the residue \(r=(1,1,1,-2)\) on a four-region seam cycle is a cocycle but not a coboundary, certified by an explicit contradiction and by the cycle pairing \(\langle z,r\rangle=-5\), invariant under the declared presentation changes, and persistent under the four declared refinement witnesses.
